...
def get_stats(path):
    ...
    patientRecords = records[records['Patient_ID'] == int(path)]
    dateList = pd.to_datetime(patientRecords['Date']).tolist()
    # convert the date to YYYY-MM-DD HH:MM format
    dateList = [datetime.strftime(i, "%Y-%m-%d %H:%M") for i in dateList]
    # store each record from the database in a list, store these lists in a dict
    records = {
        'Date': dateList, 
        'Calories_Burned': patientRecords['Calories_Burned'].tolist(),
        'Steps_Taken': patientRecords['Steps_Taken'].tolist(),
    ...
    # these loops get the list of records and turns them into a list of dicts
    # which is the required format for chartjs
    caloriesOutput = []
...
#patients stats and records are loaded in from the csv
    patients = pd.read_csv("patients.csv")
    records = pd.read_csv("records.csv")
    patientRecords = records[records['Patient_ID'] == int(path)].iloc[-1]
	#the records are then displayed in a list format 
    liveSensorReadings = {
        'Calories_Burned': patientRecords['Calories_Burned'].tolist(),
        'Steps_Taken': patientRecords['Steps_Taken'].tolist(),
        'Hours_Slept': patientRecords['Minutes_Slept'].tolist(),
	...
	#the patients details are displayed in a list format
    patientInfo = patients[patients['UniqueID'] == int(path)].iloc[0]
    patient = {
      'name': patientInfo['Name'],
      'gender': patientInfo['Gender'],
